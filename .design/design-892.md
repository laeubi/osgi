...
This document describes the design for a JDK HttpServer Whiteboard Specification. This specification aims to provide a simpler alternative to the existing Servlet Whiteboard specification by leveraging the built-in `jdk.httpserver` module (`com.sun.net.httpserver`) which has been available in the JDK since Java 6.
